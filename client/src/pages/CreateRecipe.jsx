...
export default function CreateRecipe() {
  ...
  const createRecipeMutation = useMutation({
    ...
    onError: (err) => {
      toast.error(err.response?.data?.message || 'Failed to upload recipe');
    },
  });
  ...
  const handleSubmit = (e) => {
    ...
    if (!imageFile) {
      return toast.error('Please upload a cover image for your recipe');
    }

    // Validate lists
    const filteredIngredients = ingredients.filter(ing => ing.name.trim() && ing.quantity.trim());
    if (filteredIngredients.length === 0) {
      return toast.error('Please add at least one valid ingredient (with name and quantity)');
    }
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10 animate-fade-in space-y-8">
      ...
      <div className="flex items-center gap-3 border-b border-slate-100 dark:border-slate-800 pb-5">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-800 dark:text-slate-100">Share Your Masterpiece</h1>
          <p className="text-slate-400 text-xs mt-0.5">Post a photo, write instructions, and share your cooking secrets.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-8 rounded-3xl shadow-sm space-y-6">
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Recipe Cover Image</label>
            <div
              {...getRootProps()}
              className={`h-56 rounded-2xl border-2 border-dashed cursor-pointer flex flex-col items-center justify-center relative overflow-hidden transition-all group ${
                isDragActive
                  ...
                  : imagePreview
                  ? 'border-slate-200'
                  : 'border-slate-350 hover:border-brand-500 dark:border-slate-800 dark:hover:border-brand-500 hover:bg-slate-50 dark:hover:bg-slate-850/20'
              }`}
            >
              <input {...getInputProps()} />
              {imagePreview ? (
          ...
          <div className="space-y-3">
            {ingredients.map((ing, idx) => (
              <div key={idx} className="flex gap-2 items-center animate-fade-in">
                ...
                <input
                  type="text"
                  placeholder="Unit (e.g. cups, tbsp, oz)"
                  value={ing.unit}
        ...
        {/* Instructions step-by-step dynamic section */}
        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-8 rounded-3xl shadow-sm space-y-4">
          <div className="flex justify-between items-center pb-3 border-b border-slate-50 dark:border-slate-850">
          ...
          <div className="space-y-4">
            {instructions.map((step, idx) => (
              <div key={idx} className="flex gap-3 items-start animate-fade-in">
                ...
                <textarea
                  placeholder="Describe this preparation step in detail..."
        ...
        <div className="flex justify-end gap-4">
          ...
          <button
            ...
            className="px-8 py-3 bg-brand-500 hover:bg-brand-600 text-white font-bold rounded-xl text-xs transition-all shadow-md hover:shadow-brand-500/20 flex items-center gap-2"
          >
            <Save className="h-4.5 w-4.5" />
            <span>{createRecipeMutation.isPending ? 'Publishing...' : 'Publish Recipe'}</span>
          </button>
        </div>
      </form>
    </div>
